      ...
      type water_demand_objects
        integer :: src_num = 0                  !number of the object type
      ...
      type water_receiving_objects
        integer :: rcv_num = 0                  !number of the object type
       ...
       end type water_receiving_objects
      ...
      type water_demand_objects
        ...
        integer :: ob_num = 0                   !number of the object type
        character (len=25) :: dmd_typ = ""      !selecting how to determine demand - decision table (dtbl), recall file (rec), or ave daily (ave_day)
        character (len=25) :: dtbl = ""         !decision table name to set water demand
        character (len=25) :: rec = ""          !recall input file to set water demand
        real, dimension (25) :: ave_day = 0.    !min chan flow(m3/s), min res level(frac prinicpal), max aqu depth(m)
        character (len=10) :: right = ""        !water right (sr -senior or jr - junior right)
        integer :: src_obs = 0                  !number of source objects available for the demand object
        integer :: rcv_obs = 0                  !number of receiving objects
        character (len=25) :: d_frac = ""       !decision table name to determine amount from each source
        ...
        type (hyd_output) :: hd
        type (hyd_output) :: trt
        type (water_source_objects), dimension(:), allocatable :: src       !sequential source objects as listed in wallo object
        type (water_receiving_objects), dimension(:), allocatable :: rcv    !sequential receiving objects as listed in wallo object
      end type water_demand_objects
