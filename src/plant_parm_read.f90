subroutine plant_parm_read
      ...
      character (len=3000) :: header = "" !           |header of file
      ...
      if (.not. i_exist .or. in_parmdb%plants_plt == " null") then
        ...
        tag =   in_parmdb%plants_plt
        call get_map_by_tag(in_parmdb%plants_plt, hmap, pvar, use_hdrmap)
      do
        ...
        do ic = 1, imax
          if (bsn_cc%nam1 == 0) then
            ! alt call method, read line 
            call header_read_n_reorder(104, hmap, fmt_line)
            ...
            read (fmt_line,*,iostat=eof) pldb(ic), pl_class(ic)
          end if
          if (eof < 0) exit
