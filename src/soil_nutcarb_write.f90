        ...
        do j = 1, sp_ob%hru
          ...
          do ly = 1, soil(j)%nly
            ...
            if (pcom(j)%npl > 0) then
              do ipl = 1, pcom(j)%npl
                soil_prof_root_frac = soil_prof_root_frac + pcom(j)%plg(ipl)%rtfr(ly)
              end do
            end if
            ...
            if (layer_output) then
              ...
              do ly = 1, soil(j)%nly
                write (4561,10) freq_label, ly, int(soil(j)%phys(ly)%d), time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, &
                    ...
                    pcom(j)%plg(1)%rtfr(ly), soil1(j)%root_tot(ly)%m, soil1(j)%rsd_tot(ly)%c, soil1(j)%meta(ly)%c, soil1(j)%str(ly)%c, soil1(j)%lig(ly)%c
              enddo
            endif
