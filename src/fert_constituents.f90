...
subroutine fert_constituents_apply(j, ifrt, frt_kg, fertop)
          ...
          if (size(manure_db) >= ifrt .and. ifrt > 0) then
            if (manure_db(ifrt)%path_idx > 0) then
              do ipath = 1, cs_man_db%num_paths
                path_kg = frt_kg * path_fert_ini(manure_db(ifrt)%path_idx)%ppm(ipath)
      ...
      if (cs_man_db%num_salts > 0) then
        if (allocated(salt_fert_ini)) then
          if (size(manure_db) >= ifrt .and. ifrt > 0) then
            if (manure_db(ifrt)%salt_idx > 0) then
              do isalt = 1, size(salt_fert_ini(manure_db(ifrt)%salt_idx)%ppm)
                salt_kg = frt_kg * salt_fert_ini(manure_db(ifrt)%salt_idx)%ppm(isalt)
      ...
      if (cs_man_db%num_metals > 0) then
        if (allocated(hmet_fert_ini)) then
          if (size(manure_db) >= ifrt .and. ifrt > 0) then
            if (manure_db(ifrt)%hmet_idx > 0) then
              do ihmet = 1, size(hmet_fert_ini(manure_db(ifrt)%hmet_idx)%ppm)
                hmet_kg = frt_kg * hmet_fert_ini(manure_db(ifrt)%hmet_idx)%ppm(ihmet)
      ...
      if (cs_man_db%num_cs > 0) then
        if (allocated(cs_fert_ini)) then
          if (size(manure_db) >= ifrt .and. ifrt > 0) then
            if (manure_db(ifrt)%cs_idx > 0) then
              do ics = 1, size(cs_fert_ini(manure_db(ifrt)%cs_idx)%ppm)
                cs_kg = frt_kg * cs_fert_ini(manure_db(ifrt)%cs_idx)%ppm(ics)
