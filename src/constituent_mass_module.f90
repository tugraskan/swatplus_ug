      ...
      type cs_fert_init_concentrations
        character (len=16) :: name = ""             !! name of the constituent - points to constituent database
        real, dimension (:), allocatable :: ppm    !! ppm                  |amount of constituent in manure at start of simulation
      end type cs_fert_init_concentrations
      type (cs_fert_init_concentrations), dimension(:), allocatable:: pest_fert_ini
      type (cs_fert_init_concentrations), dimension(:), allocatable:: path_fert_ini
      type (cs_fert_init_concentrations), dimension(:), allocatable:: hmet_fert_ini
      !! first 8 values of soil and plt are salt ion concentrations and next 5 are salt mineral fractions
      type (cs_fert_init_concentrations), dimension(:), allocatable:: salt_fert_ini
      type (cs_fert_init_concentrations), dimension(:), allocatable:: cs_fert_ini !rtb cs
