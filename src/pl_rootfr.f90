    ...
    integer, intent (in) :: ipl_in    !none               |plant number
    integer :: k = 0                  !            |
    ...
    if (pcom(j)%plg(ipl_in)%root_dep < 1.e-6) then
      pcom(j)%plg(ipl_in)%rtfr(ly) = 1.
      return
    endif
    ...
    do ly = 1, soil(j)%nly
      if (soil(j)%phys(ly)%d >= pcom(j)%plg(ipl_in)%root_dep) then
        cum_rd = pcom(j)%plg(ipl_in)%root_dep
      else
        cum_rd = soil(j)%phys(ly)%d
      end if
      !cum_d = cum_d + soil(j)%phys(ly)%thick
      !if (cum_d >= pcom(j)%plg(ipl_in)%root_dep) cum_rd = pcom(j)%plg(ipl_in)%root_dep
      !if (cum_d < pcom(j)%plg(ipl_in)%root_dep) cum_rd = cum_d
      
      x1 = (cum_rd - soil(j)%phys(ly)%thick) / pcom(j)%plg(ipl_in)%root_dep
      x2 = cum_rd / pcom(j)%plg(ipl_in)%root_dep
      xx1 = -b * x1
      if (xx1 > 20.) xx1 = 20.
      xx2 = -b * x2
      if (xx2 > 20.) xx2 = 20.
      pcom(j)%plg(ipl_in)%rtfr(ly) = (a/b*(Exp(xx1) - Exp(xx2)) + c *(x2 - x1))/d
      xx = cum_rf
      cum_rf = cum_rf + pcom(j)%plg(ipl_in)%rtfr(ly)
      if (cum_rf > 1.) then
        pcom(j)%plg(ipl_in)%rtfr(ly) = 1. - xx
        cum_rf = 1.0
      end if
      k = ly
      if (cum_rd >= pcom(j)%plg(ipl_in)%root_dep) Exit
          
    end do

    !!   ensures that cumulative fractional root distribution = 1
    do ly = 1, soil(j)%nly
      pcom(j)%plg(ipl_in)%rtfr(ly) = pcom(j)%plg(ipl_in)%rtfr(ly) / cum_rf
